...
path = "./safe_act_dqn"  # The path to save our model to.
...
if net_type == 1:
    ...
    left_turn_lanes = ['4i_1', '2i_1', '3i_1', '1i_1']
    opposing_lanes = {
        '1i_1': ['2i_0', '2i_1'],
        '2i_1': ['1i_0', '1i_1'],
        '3i_1': ['4i_0', '4i_1'],
...
def safety_check(veh_id, check="RED_LIGHT_VIOLATION"):
    need_correct = []
    if check == "RED_LIGHT_VIOLATION":
        lanes_in = traci.vehicle.getLaneID(veh_id)
        length = traci.lane.getLength(lanes_in)
        for link_info in traci.lane.getLinks(lanes_in):
            v_0 = traci.vehicle.getSpeed(veh_id)
            if not link_info[2] and v_0 > 8:  # if not open
                safe_stopping_distance = 1.47 * v_0 * 2.236936 * tr * 0.3048
                if link_info[3] and length - traci.vehicle.getLanePosition(veh_id) < safe_stopping_distance:
                    return True
    if check == "PERMITTED_LEFT_TURN":
        if net_type == 1:
            for left_turn_lane in left_turn_lanes:
                link_info = traci.lane.getLinks(left_turn_lane)[0]
                link_info_1 = traci.lane.getLinks(left_turn_lane)[0]
                if traci.lane.getLastStepOccupancy(left_turn_lane) > 0:
                    if link_info[5] == 'g' and link_info[3]:
                        return True

                    if link_info_1[5] == 'g' and link_info_1[3]:
                        return True
        else:
            for left_turn_lane in left_turn_lanes:
                link_info = traci.lane.getLinks(left_turn_lane)[1]
                link_info_1 = traci.lane.getLinks(left_turn_lane)[2]
                if traci.lane.getLastStepOccupancy(left_turn_lane) > 0:
                    if link_info[5] == 'g' and link_info[3]:
                        return True
...
for i in range(1, num_episodes):
    ...
    while j < max_epLength:
        ...
        s1, r, d, wait_time, collision_ph, collisions, r_eff, cf_result = take_action_safe_act(i, j, None, safety_check, collisions, tls, a, wait_time, wait_time_map, safe_weight, net_type)
    ...
    if net_type == 1:
        df.to_csv('safe_act_sync.csv')
    elif net_type == 0:
        df.to_csv('safe_act_real.csv')
