...
class CTRLG:
    ...
    def generate_counterfactuals(self, data, coll, action_space=10):
        ...
        for i in range(action_space):
            # 获取不等于3,4,8,9的索引
            exclude_actions = torch.tensor([i], device=self.device)
            mask = ~torch.isin(actions.squeeze(), exclude_actions)
            valid_indices = torch.where(mask)[0]
            if len(valid_indices) > 0:
                # 获取原始动作
                original_actions = actions[valid_indices]
                # 创建新的动作张量
                new_actions = original_actions.clone()
                
                # 应用映射规则
                # for i in range(len(new_actions)):
                #     action = new_actions[i].item()
                #     if action in [0, 1, 2]:
                #         new_actions[i] = 3
                ...
                # 转换为one-hot编码
                new_actions_one_hot = torch.nn.functional.one_hot(new_actions.squeeze(), self.action_dim).float().view(1,-1)
                
                # 生成新的状态和奖励
                with torch.no_grad():
                    new_next_states, new_rewards = self.generator(
                        states[valid_indices], 
                        new_actions_one_hot, 
                        noise_s[valid_indices], 
                        noise_r[valid_indices]
                    )
                
                # 调整奖励
                new_rewards[:, -2] = (new_rewards[:, -2] - coll[valid_indices])
